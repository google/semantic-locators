...
describe('findElementsBySemanticLocator', () => {
  ...
  describe('implicit role', () => {
    ...
    it('for attribute_takes_value', () => {
      render(
        html`<input type="button" id="foo" /><input type="text" />`,
        container,
      );
    ...
    it('for attribute_does_not_take_value', () => {
      render(
        html`<table
          ><tr><th id="foo" scope="something">h</th></tr></table
        >`,
        container,
      );

      expect(
        findElementsBySemanticLocator('{columnheader}', container),
    ...
    it('for non_empty_attribute', () => {
      render(
        html`<img src="data:," id="foo" alt="bar" />
          <img src="data:," alt="" />`,
        container,
      );
    ...
    it('for parent_tag', () => {
      render(
        html` <datalist style="display:block;">
    ...
    it('for not_ancestor_role', () => {
      render(
        html`<header id="foo">xxx</header
          ><div role="complementary"><header>yyy</header></div>`,
        container,
      );
    ...
    it('for has_accessible_name', () => {
      render(
        html`<section role="region" aria-label="something" id="foo"></section
          ><section></section>`,
        container,
      );

      expect(findElementsBySemanticLocator('{region}', container)).toEqual([
        document.getElementById('foo')!,
      ]);
    });
        ...
        html`
          <table
            ><tr
              ><td>
                <table role="grid"
                  ><tr><td id="foo">aaaa</td></tr></table
                > </td
              ><td>
                <table
                  ><tr><td>aaaa</td></tr></table
                >
              </td></tr
            ></table
          >
    ...
    it('for data_in_column and data_in_row', () => {
      render(
        html`
          <table>
            <tr
              ><th id="ch_0">h</th><th id="ch_1">h</th><th id="ch_2">h</th></tr
            >
            <tr
              ><th id="rh_0">h</th><th id="cell_0">h</th
              ><td id="cell_1">d</td></tr
            >
            <tr
              ><th id="rh_1">h</th><td id="cell_2">d</td
              ><td id="cell_3">d</td></tr
            >
          </table>
        `,
        container,
  ...
  describe('accessible name', () => {
    ...
    it('matches on name from content', () => {
      ...
      expect(findElementsBySemanticLocator('{button "OK"}', container)).toEqual(
        [document.getElementById('foo')!],
      );
    });
    ...
    it('accepts leading wildcards', () => {
      ...
      expect(
        findElementsBySemanticLocator('{button "* is fine"}', container),
      ).toEqual([document.getElementById('foo')!]);
    });
    ...
    it('accepts trailing wildcards', () => {
      ...
      expect(
        findElementsBySemanticLocator('{button "This is *"}', container),
      ).toEqual([document.getElementById('foo')!]);
    });
    ...
    it('handles non-ASCII characters', () => {
      render(
        html`<button
          id="foo"
          aria-label='блČλñéç‪हिन्दी‬日本語‬‪한국어‬й‪ไ🤖-—–;|<>!"_+'
          >OK</button
        >`,
        container,
      );
  ...
  it('finds by explicit ARIA attributes', () => {
    render(
      html` <div role="button" aria-disabled="true" id="foo">xxx</div>
        <div role="button">yyy</div>`,
      container,
    );

    expect(
      findElementsBySemanticLocator('{button disabled:true}', container),
    ).toEqual([document.getElementById('foo')!]);
  });

  it('finds by implicit ARIA attributes', () => {
    render(
      html` <h1 id="foo">Foo</h1>
        <h2>Bar</h2>`,
      container,
    );

    expect(
      findElementsBySemanticLocator('{heading level:1}', container),
    ).toEqual([document.getElementById('foo')!]);
  });
  ...
  it('ignores inner nodes if "outer" is at the start', () => {
    ...
    expect(
      findElementsBySemanticLocator('outer {listitem}', container),
    ).toEqual([document.getElementById('foo')!]);
  });
  ...
  it('ignores inner nodes if "outer" is in the middle', () => {
    ...
    expect(
      findElementsBySemanticLocator('{listitem} outer {list}', container),
    ).toEqual([
      document.getElementById('foo')!,
  ...
  it('finds elements with a chain of SemanticNodes', () => {
        ...
        ><li>
          <ul
            ><li>
              <table
                ><tr
                  ><td><button id="foo">xxx</button></td></tr
                ></table
              >
              <table
                ><tr><td></td></tr
    ...
    expect(
      findElementsBySemanticLocator(
        '{listitem} outer {listitem} {table} {button}',
        container,
      ),
    ).toEqual([document.getElementById('foo')!]);
  });
...
describe('findElementBySemanticLocator', () => {
  it('returns the first matching element in document order', () => {
    ...
    expect(findElementBySemanticLocator('{listitem}', container)).toEqual(
      document.getElementById('foo')!,
    );
  });
